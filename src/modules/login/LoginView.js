  ...
  initializeIfNeeded = async () => {
    if (!(await Spotify.isInitializedAsync())) {
      ...
      const options = {
        ...
        tokenSwapURL: 'https://react-native-spotify-server.herokuapp.com/swap',
        tokenRefreshURL:
      ...
      if (session && session.expireTime - Date.now() < 0) {
        ...
        console.log('session does not need to be renewed / does not exist');
      }
    ...
    } else if (await Spotify.isLoggedInAsync()) {
      this.props.navigation.navigate('Main');
    } else {
      this.setState({loaded: true});
    }
